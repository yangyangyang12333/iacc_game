...
print(os.getcwd())
os.chdir('/mnt/sdc/zy/demo/zero_shot_baseline')


# train
def have_set_baseline(args):
    '''self set
    如果使用sh进行运行则将引用函数语句进行注释'''
    run_ID = 'baseline'# 一个名字
    DATA = './'
    TRAINER = 'Caption_distill_double'

    DATASET = 'pazhou_distill_chatglm' # 使用的数据集
    CFG = 'rn50' # 使用的backbone
    CTP = 'end' # token的位置
    NCTX = 16 # conext tokens 的位置
    CSC = False
    SEED = 1
    ...
    args.config_file = f'configs/trainers/{TRAINER}/{CFG}.yaml'
    args.output_dir = f'output/{run_ID}/{TRAINER}/{CFG}/nctx{NCTX}_csc{CSC}_ctp{CTP}/seed{SEED}'

    args.opts = ['TRAINER.Caption.N_CTX', f'{NCTX}',
                 'TRAINER.Caption.CSC', f'{CSC}',
                 'TRAINER.Caption.CLASS_TOKEN_POSITION', f'{CTP}']

    pass

def have_set_baseline_addCSC(args):
    '''self set
    如果使用sh进行运行则将引用函数语句进行注释'''
    run_ID = 'baseline_addCSC'# 一个名字
    DATA = './'
    TRAINER = 'Caption_distill_double'
...
if __name__ == "__main__":
    ...
    have_set_baseline(args) # 如果使用sh进行运行则进行注释
    main(args)
