...
class TextEncoder(nn.Module):
    ...
    def forward(self, prompts, tokenized_prompts, if_embedding=True, if_sequence=False):
        if not if_embedding:
            ...
            prompts = self.token_embedding(prompts).type(self.dtype)# 进行编码  # [batch_size, n_ctx, d_model]
        x = prompts + self.positional_embedding.type(self.dtype)# 位置编码
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        prompts = [prompt_prefix + " " + name + "." for name in classnames]# list(str)

        tokenized_prompts = torch.cat([clip.tokenize(p) for p in prompts])# (cls, vocab_size)
        with torch.no_grad():
            embedding = clip_model.token_embedding(tokenized_prompts).type(dtype)# (n_cls, transformer_width)
...
class Caption_distill_double(TrainerX):
    ...
    def test(self, split=None):
        ...
        results = self.evaluator.evaluate(cfg.OUTPUT_DIR)
